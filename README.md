...
You can create a singularity container from the docker image.
    ```
    singularity build general-annot.simg docker://alaine1/gen-annot:1.0.3
    ```
...
- **Step 1: Retrieve the docker image.**
    ```
    docker pull alaine1/gen-annot:1.0.3
    ```
- **Step 2: Run**.
    ...
    ```
    sudo docker run -v ${PWD}/config.json:/annot/config.json -v /store:/store alaine1/gen-annot:1.0.3
    ```
